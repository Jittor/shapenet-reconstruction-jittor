...
def train():
    ...
    for i in range(start_iter, args.num_iterations + 1):
        ...
        if run_times != -1:
            if cnt == run_times:
                jt.sync_all(True)
                print(f"Costs {time.time() - sta} secs for running {run_times} times.")
                exit(0)
            else:
                cnt += 1
                continue
        ...
        if i % args.save_freq == 0:
            model_path = os.path.join(directory_output, 'checkpoint_%07d.pkl'%i)
        ...
        if i % args.demo_freq == 0:
            ...
            imageio.imsave(os.path.join(image_output, '%07d_fake.png' % i), img_cvt(render_images[0][0][None,:,:]))
        ...
        if i % args.print_freq == 0:
            print('Iter: [{0}/{1}]\t'
                  ...
                  'Loss {loss:.3f}\t'
                  'lr {lr:.6f}\t'
                  'sv 1\t'.format(i, args.num_iterations,
                                         batch_time=batch_time.val, loss=losses.val.tolist()[0], 
